...
void IOBuff::ReadFile(const std::string &path) {
  std::ifstream ifs(path);
  if (!ifs) {
    // コンフィグファイルのオープンエラーは、致命的なので例外を投げ、mainでcatchして終了する
    // それ以外のファイルのオープンエラーは、プログラムは継続するので適宜例外をcatchして処理する
